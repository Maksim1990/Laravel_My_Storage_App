    ...
    <div class="col-sm-8 col-sm-offset-1 col-lg-8 col-lg-offset-1" style="padding-top: 30px;">
        <div class="col-sm-12 col-xs-12" style="margin-bottom: 100px;">
                    ...
                    @else
                        <div style='border-radius: 20px;'>
                            <p  id='add_image' class='w3-hover-opacity' style="height:200px;background-repeat: no-repeat;background-size: 250px 250px;background-image:url('{{"/images/includes/noImage.jpg"}}')">
                                @if($movie->user_id==Auth::id() || Auth::user()->role_id=='1')
                                    <a href="{{URL::to('/'.LaravelLocalization::getCurrentLocale().'/movies/'.$movie->id."/edit/image")}}" style='height:100%;text-decoration:none;'>
            ...
            <div class="col-sm-6 col-xs-12" id="book_details">
                <p>@lang_u('messages.book') ID: {{$movie->id}}</p>
                <p>@lang_u('messages.title'): {{$movie->title}}</p>
                <p>@lang_u('messages.author'): {{$movie->author}}</p>
                <p>Added by:
                    <a href="{{URL::to('/'.LaravelLocalization::getCurrentLocale() .'/users/'.$movie->user->id)}}">
                        {{$movie->user->name}}</a></p>
            </div>
...
@section('scripts')
    ...
    <script>
        ...
        //-- Add to favorite functionality
        $(".like").click(function () {
            var urlFavorite = '{{ URL::to('add_movie_to_favorite_ajax') }}';
