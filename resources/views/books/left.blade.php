<div class="w3-center " id="user_left">
    @if($book->user_id==Auth::id())
        ...
        <p>
            <a href="{{URL::to('/'.LaravelLocalization::getCurrentLocale().'/books/'.$book->id."/edit/image")}}">@lang('messages.change_main_image')</a>
        </p>
        <div>
    ...
    <div id="rating_block" class="col-sm-12">
        ...
        <div id="rating_message_block" class="w3-center">
            <span id="rating_message" style="display:{{$blnDisplayRateMessage}};"
                  class="w3-text-green">@lang('messages.thank_you_for_voting')</span>
        </div>

        <div id="rating_statistics_block">
            <p id="rating_statistics_message" class="w3-text-green w3-center w3-tiny">
                @if(count($ratings)>0)
                    <a href="#" onclick="document.getElementById('id07').style.display='block'">
                        <span id="voteCount"
                              class="w3-medium">{{$countRating}}</span> @lang('messages.people_already_voted')
                    </a>
                @endif
            </p>
        </div>
